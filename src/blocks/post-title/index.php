...
?>
    ...
    if (empty($wrapperTag)) : ?>
      ...
      <?php if ($postTitleIsLink) : ?>
        <a class="<?php echo esc_attr($blockId); ?> <?php echo esc_attr($postTitleClass); ?>" href="<?php echo !empty($customUrl) ? esc_url_raw($customUrl) : esc_url_raw($post_url); ?>" rel="<?php echo esc_attr($rel); ?>" target="<?php echo esc_attr($linkTarget); ?>" <?php /* TO code reviewers, $linkAttrStr escaped correctly before, No need here.*/ echo $linkAttrStr; ?>>
          <?php if (!empty($prefixText)) : ?>
            <span class="<?php echo esc_attr($prefixClass); ?>"> <?php echo wp_kses_post($prefixText); ?> </span>
          <?php endif; ?>
          <?php echo wp_kses_post($post_title); ?>
