...
import MyLazy from "./lazyPlaceholder.gif";
	...
	icon: {
		...
		src: (
			<svg
				...
				<path
					d="M21.96 5.99976H137.04C139.78 5.99976 142 8.2205 142 10.9587V126.027C142 128.765 139.78 130.986 137.04 130.986H21.96C19.2201 130.986 17 128.765 17 126.027V10.9587C17 8.2205 19.2201 5.99976 21.96 5.99976Z"
					stroke="url(#paint0_linear_61_233)"
					stroke-width="12"
				/>
				<path
					d="M146 144.657H13C11.8954 144.657 11 145.553 11 146.657V158C11 159.104 11.8954 160 13 160H146C147.105 160 148 159.104 148 158V146.657C148 145.553 147.105 144.657 146 144.657Z"
					fill="url(#paint1_linear_61_233)"
				/>
				<path
					d="M88.454 44.9314L65.7519 81.3817L56.7072 66.9683L32.9199 105.205H50.9187H80.5855H126.08L88.454 44.9314Z"
					fill="#C15940"
				/>
				<defs>
					<linearGradient
		...
		function onPickBlockPatterns(content, action) {
			...
			if (action == "applyStyle") {
				...
				if (imageX != undefined) {
					...
					blockCssObj[imageSelector] = imageY;
				}
				...
				<InspectorControls>
					<div className="px-3 pg-setting-input-text">
						...
						>
							<PanelRow>
								<ToggleControl
									label="Lazy Load Enable?"
									help={
										image.options.lazy
											? "Lazy Load Enabled"
											: "Lazy Load Disabled."
									}
									checked={image.options.lazy ? true : false}
									onChange={(e) => {
										var options = {
											...image.options,
											lazy: image.options.lazy ? false : true,
										};
										setAttributes({
											image: { ...image, options: options },
										});
							...
							{image.options.lazy && (
								<div className="pt-3">
									<PanelRow>
										...
										<SelectControl
											label=""
											value={image.options.lazySrcType}
											options={[
												{ label: "Media", value: "media" },
												// { label: "Custom Field", value: "customField" },
												{ label: "Image Source URL", value: "customUrl" },
											...
											onChange={(newVal) => {
												var options = { ...image.options, lazySrcType: newVal };
												setAttributes({
													image: { ...image, options: options },
												});
											}}
									...
									{image.options.lazySrc.length !== undefined &&
										image.options.lazySrc.length > 0 && (
											<MediaUploadCheck>
												<MediaUpload
													className="bg-blue-500"
													onSelect={(media) => {
									...
									{image.options.lazySrcType == "media" && (
										<>
											<div className="mt-5" for="">
												Choose Image
											</div>
									...
									{image.options.lazySrcType == "customUrl" && (
										<>
											<PanelRow>
												<label for="" className="font-medium text-slate-900 ">
													Image URL
												...
												<div className="relative">
													...
													{image.options.lazySrc.length > 0 && (
														<Button
															...
															icon={linkOff}
															onClick={(ev) => {
																var options = { ...image.options, lazySrc: "" };
																setAttributes({
																	image: { ...image, options: options },
																});
									...
									{/* {image.options.lazySrcType == "imgId" && (
									<PanelRow>
										<label for="" className="font-medium text-slate-900 ">
											Image ID
										</label>
