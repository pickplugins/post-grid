...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    function ajaxPageClick() {
        ...
        if (paginationAjaxLink != null) {
            paginationAjaxLink.forEach(el => el.addEventListener('click', event => {

                event.preventDefault();
                var href = event.target.getAttribute("href");

                var hrefParts = href.split('/');
                var filtered = hrefParts.filter(function (el) { return el; });
                var lastElement = parseInt(filtered.pop());


                var blockargs = el.closest('div[blockArgs]').getAttribute("blockargs")
                    ...
                    .then((response) => {
                        if (response.ok && response.status < 400) {
                            response.json().then((dataX) => {
                                ...
                                var posts = (dataX['posts'] != undefined) ? dataX['posts'] : [];
                                var pagination = (dataX['pagination'] != undefined) ? dataX['pagination'] : [];
                                ...
                                setTimeout(() => {
                                    itemsLoopWrap.scrollIntoView({ behavior: "smooth", block: "start", inline: "nearest" });
                                    setTimeout(() => {
    ...
    loadMorewrap != null && loadMorewrap.addEventListener('click', function () {
                    ...
                    if (response.ok && response.status < 400) {
                        response.json().then((dataX) => {
                            ...
                            if (!noPosts) {
                                ...
                                itemsLoopWrap.insertAdjacentHTML('beforeend', html);
                                loadMorewrap.scrollIntoView({ behavior: "smooth", block: "start", inline: "nearest" });
                            }
