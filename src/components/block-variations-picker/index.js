			...
			<div className="m-auto  " id={props.blockName}>
				...
				{cssLibrary.items.slice(0, 10).map((x, index) => {
					...
					return (
						<>
							<div
								className={`item-${index} variation-item border border-solid relative border-slate-400 rounded-md overflow-hidden hover:border-black hover:shadow-md hover:shadow-slate-300 transition-all duration-150 ease-in-out shadow-md py-2 pt-3 mb-4  `}
								onClick={(ev) => {
									if (!x.is_pro) {
								...
								{x.is_pro && isProFeature && (
									<div className="flex justify-center mt-4">
										<button
											className="px-3 py-1 border-0 bg-amber-500 rounded-sm text-white outline-none focus:ring-4 shadow-lg transform active:scale-75 transition-transform text-[16px] flex items-center gap-2 justify-center "
											onClick={(ev) => {
												window.open(
